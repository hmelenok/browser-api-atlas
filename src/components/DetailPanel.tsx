import {ExternalLink, X, Sparkles, BookOpen, FileCode2, MessageSquare} from 'lucide-react'
...
export function DetailPanel() {
  ...
  return (
    <>
      {/* Backdrop tap target for outside-click close on mobile */}
      <div
        className={cn(
      ...
      <aside
        ...
        aria-hidden={!isOpen}
      >
        {entry && <PanelContents key={entry.id} entryId={entry.id} runtime={runtime[entry.id]} onClose={() => select(null)} />}
      </aside>
    </>
  )
}
...
function PanelContents({
  ...
  entryId: string
  runtime: {supported: boolean; reason?: string} | undefined
  onClose: () => void
}) {
        ...
        <section className="border-b border-[var(--color-border)] px-5 py-4">
          <div className="grid grid-cols-2 gap-3 text-xs">
            <Stat label="In your browser">
              <span style={{color: runtime?.supported ? 'var(--color-status-supported)' : 'var(--color-status-unsupported)'}}>
                {runtime?.supported ? 'Yes' : 'No'}
              </span>
              {!runtime?.supported && runtime?.reason && (
                <span className="ml-1 text-[10px] text-[var(--color-muted)]">({runtime.reason})</span>
              )}
            </Stat>
